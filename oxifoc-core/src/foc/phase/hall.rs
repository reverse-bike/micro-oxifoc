//! Calibrated Hall electrical-angle tracking in Q0.32 turns.

use crate::foc::angle::Turns;
use crate::foc::numeric::Fixed;
use crate::foc::phase::{ControlPhaseEstimate, ControlPhaseProvider, PhaseStrategy};
...
impl ControlPhaseProvider<Fixed> for HallTracker {
    type Angle = Turns;
    ...
    fn estimate(&self, elapsed_since_edge_us: u32) -> Option<ControlPhaseEstimate<Turns>> {
        self.angle(elapsed_since_edge_us)
            .map(|angle| ControlPhaseEstimate {
                angle,
...
mod tests {
    ...
    fn tracker() -> HallTracker {
        HallTracker::new(&TEST_GEOMETRY)
    }
