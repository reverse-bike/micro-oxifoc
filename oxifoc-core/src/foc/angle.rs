//! Electrical-angle and trigonometry backends for the shared controller.
...
pub trait AngleSinCos<N: Scalar> {
    type Angle: Copy;
...
pub struct CordicTurns;

impl AngleSinCos<Fixed> for CordicTurns {
    type Angle = Turns;
...
#[derive(Clone, Copy, Debug, Default)]
pub struct LibmRadians;

#[cfg(feature = "algorithms")]
impl AngleSinCos<f32> for LibmRadians {
    type Angle = f32;
...
mod tests {
    ...
    fn fixed_and_float_backends_agree_around_a_turn() {
        for eighth in 0..8_u32 {
            ...
            let (fixed_sin, fixed_cos) = CordicTurns::sin_cos(turns);
            let (float_sin, float_cos) = LibmRadians::sin_cos(radians);
